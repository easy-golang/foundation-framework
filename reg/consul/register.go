...
func RegisterService(listenOn string, c consul.Conf) error {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				for {
					if err := client.Agent().ServiceRegister(reg); err != nil {
						logx.Errorf("attempt register service '%s' host to consul error: %s", c.Key, err.Error())
						time.Sleep(time.Second * 3)
						continue
					}
					logx.Infof("re-registration service %s success", c.Key)
					break
				}
			}
		}
